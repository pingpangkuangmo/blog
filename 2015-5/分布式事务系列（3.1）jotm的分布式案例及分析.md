...
上述jotm-datasource主要是为了将上述StandardXAPoolDataSource数据源放置到容器中，如tomcat，而不是应用程序中。应用程序通过JNDI的方式来tomcat容器中获取上述数据源。所以对于本工程来说可以不要，对于下文说的通过JNDI方式使用jotm则是必须的
...
我们来仔细研究下，它到底用的是什么dataSource，来看下org.objectweb.jotm.datasource.DataSourceFactory的内容：
...
我们继续看下jotm是如何来的？这里正是jotm-core-2.3.1-M1.jar出现bug的地方：
...
解决办法就是换成低版本的jotm-core，如2.2.2版本就可以了，不会产生上述问题。
