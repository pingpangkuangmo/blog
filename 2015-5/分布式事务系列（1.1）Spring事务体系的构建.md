...
-	1 第一个接口：TransactionStatus getTransaction(TransactionDefinition definition) 根据事务定义获取事务状态
	...
	大体内容就是先获取上述说明的Object transaction，然后就是new DefaultTransactionStatus,然后使用Object transaction开启事务，最后返回DefaultTransactionStatus。
...
至此事务管理器接口就简略说完了，还有很多细节的东西，如事务传播属性的处理策略，需要各位再去仔细研究。
