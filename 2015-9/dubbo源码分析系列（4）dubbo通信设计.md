...
HeaderExchangeClient和上面的非常类似，就不再提了。
...
-	1 DubboProtocol中有一个如下结构
	...
	会先根据服务的url获取要发布的服务所在的host和port，以此作为key来从上述结构中寻找是否已经有对应的ExchangeServer（上面已经说明）。
...
-	2 如果没有的话，则会创建一个，创建过程如下：
	...
	所以此时我们可以自定义扩展功能来实现ExchangeServer。接口定义如下：
	...
	默认使用的就是HeaderExchangeServer如下所示：
