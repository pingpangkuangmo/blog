...
-	5  java内存模型以及垃圾回收算法
	...
	-	5.6 常用gc的参数：-Xmn、-Xms、-Xmx、-XX:MaxPermSize、-XX:SurvivorRatio、-XX:-PrintGCDetails
...
-	6  锁以及并发容器的源码
	...
	-	6.10 HashMap、HashTable、ConcurrentHashMap、TreeMap的实现原理
...
-	5 sql中死锁
	...
	-	5.1 深入理解mysql的Record Locks、Gap Locks、Next-Key Locks
...
-	7 SpringSecurity、shiro、SSO（单点登录）
	...
	- 7.1 SpringSecurity的认证过程以及与Session的关系
	- 7.2 CAS实现SSO
...
-	11 扩展
	...
	![输入图片说明](https://static.oschina.net/uploads/img/201607/25230627_53xn.png "在这里输入图片标题")
...
-	5 Hive
	...
	-	5.1 HiveServer2的thrift RPC通信过程
	-	5.2 Hive的优化过程
	
-	6 Hbase

	-	6.1 Hbase的整体架构图
	-	6.2 Hbase的WAL和MVCC设计
